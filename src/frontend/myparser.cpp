...
extern void set_scanner_input(
    const char
        *filename); // tell the scanner to scan characters from "filename"
extern void close_scanner_input();
std::string TokenName[] = { // string name for enumeration TokenType
    "identifier", "int_type", "int_const", "return", "(",    ")",  "{", "}",
    ";",          "=",        "?",         ":",      "and",  "or", "<", ">",
    "<=",         ">=",       "==",        "!=",     "+",    "-",  "*", "/",
    "%",          "~",        "!",         "if",     "else", "end"};
...
// !NOTE: isFirst and isFollow is not complete, please modify the arrays as
// needed
bool isFirst[MAX_Symbol][MAX_Token];
bool isFollow[MAX_Symbol][MAX_Token];
static void init_first_set() {
    /*
   isFirst[SvmbolType::Primary][next.token.type]用来判断下一个token 是否在
  FIRST(primary)中,以选择产生式unary:primary。
  在同学们需要填空的代码中，待选择的多个产生式右端通常都以终结符开始，因此只需通过判断
  next token 的终结符类型选择产生式即可，
  无需用到isFirst集合。同时请注意【目前的isFirst集合并不完整】，如果想使用，请按照自己需求【初始化对应项的值】。
  */
    memset(isFirst, 0, sizeof(isFirst));
    // incrementally build first set for
    // Primary/Unary/Multiplicative/Additive/Relational
    ...
    memcpy(isFirst[SymbolType::Unary], isFirst[SymbolType::Primary],
           sizeof(isFirst[0]));
    ...
    memcpy(isFirst[SymbolType::Multiplicative], isFirst[SymbolType::Unary],
           sizeof(isFirst[0]));

    memcpy(isFirst[SymbolType::Additive], isFirst[SymbolType::Multiplicative],
           sizeof(isFirst[0]));

    memcpy(isFirst[SymbolType::Relational], isFirst[SymbolType::Additive],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::Equality], isFirst[SymbolType::Relational],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::LogicalAnd], isFirst[SymbolType::Equality],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::LogicalOr], isFirst[SymbolType::LogicalAnd],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::Conditional], isFirst[SymbolType::LogicalOr],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::Assignment], isFirst[SymbolType::Conditional],
           sizeof(isFirst[0]));
    memcpy(isFirst[SymbolType::Expression], isFirst[SymbolType::Assignment],
           sizeof(isFirst[0]));

    memcpy(isFirst[SymbolType::Statement], isFirst[SymbolType::Expression],
           sizeof(isFirst[0]));
...
//  for example, p_VarDecl() recursively parse a VarDecl symbol and return a
//  `VarDecl` node if success.
...
// you can access next_token.type directly without calling lookahead(), thus not
// consuming the token. notice when to consume and when not to consume the
// token.
...
ast::Program *MindCompiler::parseFile(const char *filename) {
    ...
    if (next_token.type != TokenType::END) {
        mind::err::issue(
            next_token.loc,
            new mind::err::SyntaxError("extra garbage at the end of file"));
    }
    close_scanner_input();
    return ptree;
}
...
Token lookahead(TokenType t) {
    // consume one token with TokenType t(throw error if failed), and get the
    // next token from scanner to check the type of next_token without consuming
    // it, access next_token.type directly
    Token tmp =
        next_token; // next_token: global variable declared before parseFile()
    if (next_token.type == t) {
        next_token = yylex();
        return tmp;
    }
    mind::err::issue(next_token.loc, new mind::err::SyntaxError(
                                         "expect " + TokenName[t] + " get " +
                                         TokenName[next_token.type]));
    return tmp;
}
Token lookahead() { // when called without specifying a TokenType, we just jump
                    // the current token and get the next token
...
static ast::FuncDefn *p_Function() {
    ...
    return new ast::FuncDefn(name.id, return_type, new ast::VarList(),
                             statements, return_type->getLocation());
}
...
static ast::Type *p_Type() {
    ...
    // 2. Build a `IntType` node and return it. Refer to ast/ast.hpp for
    // detailed information.

    Token token = lookahead(TokenType::INT);
    return new ast::IntType(token.loc);
}
...
static ast::StmtList *p_StmtList() {
    ...
    if (!isFirst[SymbolType::Statement][next_token.type] &&
        ...
        mind::err::issue(next_token.loc,
                         new mind::err::SyntaxError(
                             "unexpected " + TokenName[next_token.type]));
        lookahead();
    }
    while (!isFollow[SymbolType::StmtList][next_token.type]) {
        if (isFirst[SymbolType::Statement][next_token.type]) {
            ...
            ast::Statement *stmt = p_Statement();
            statements->append(stmt);
        } else if (isFirst[SymbolType::VarDecl][next_token.type]) {
            // TODO: Complete the action if the next is a declaration. Remeber
            // to consume ';'.
            ast::VarDecl *decl = p_VarDecl();
            statements->append(decl);
            lookahead(TokenType::SEMICOLON);
        } else {
            mind::err::issue(
                next_token.loc,
                new mind::err::SyntaxError("expect statement or vardecl, get" +
                                           TokenName[next_token.type]));
        }
    }
    return statements;
...
static ast::Statement *p_Statement() {
    ...
    if (isFirst[SymbolType::Expression][next_token.type]) {
        ...
        // `If`/`LBRACE`/`SEMICOLON` else if (...) {} else if (...) {}
    ...
    mind::err::issue(next_token.loc,
                     new mind::err::SyntaxError("expect statement, get" +
                                                TokenName[next_token.type]));
    return NULL;
}
...
static ast::ReturnStmt *p_Return() {
     ...
     **/
    Token token = lookahead(TokenType::RETURN);
    ast::Expr *expr = p_Expression();
    lookahead(TokenType::SEMICOLON);
    return new ast::ReturnStmt(expr, token.loc);
}
...
static ast::IfStmt *p_If() {
     ...
     * 6. If the next token is 'Else', match token 'else' and parse statement to
     *get otherwise of the node.
     ...
     **/
    Token token = lookahead(TokenType::IF);
    lookahead(TokenType::LPAREN);
    ast::Expr *cond = p_Expression();
    lookahead(TokenType::RPAREN);
    ast::Statement *body = p_Statement();
    if (next_token.type == TokenType::ELSE) {
        lookahead(TokenType::ELSE);
        ast::Statement *otherwise = p_Statement();
        return new ast::IfStmt(cond, body, otherwise, token.loc);
    }
    return new ast::IfStmt(cond, body, new ast::EmptyStmt(token.loc), token.loc);
}
...
static ast::Expr *p_Assignment() {
    ...
    if (next_token.type == TokenType::ASSIGN) {
        ...
        return new ast::AssignExpr(dynamic_cast<ast::LvalueExpr *>(node)->lvalue, rhs,
                                   node->getLocation());
    }
    return node;
}
...
static ast::Expr *p_LogicalAnd() {
    ...
    while (next_token.type == TokenType::AND) {
        Token And = lookahead();
        ast::Expr *e2 = p_Equality();
        node = new ast::AndExpr(node, e2, And.loc);
    }
    return node;
}
...
static ast::Expr *p_Equality() {
    ...
    while (next_token.type == TokenType::EQU ||
           next_token.type == TokenType::NEQ) {
...
static ast::Expr *p_Relational() {
    ...
    while (
        next_token.type == TokenType::LEQ || next_token.type == TokenType::LT ||
        next_token.type == TokenType::GEQ || next_token.type == TokenType::GT) {
        ...
        switch (op.type) {
        case TokenType::LT:
            node = new ast::GrtExpr(operand2, node, op.loc);
            break;
        case TokenType::LEQ:
            node = new ast::GeqExpr(operand2, node, op.loc);
            break;
        case TokenType::GT:
            node = new ast::GrtExpr(node, operand2, op.loc);
            break;
        case TokenType::GEQ:
            node = new ast::GeqExpr(node, operand2, op.loc);
            break;
        default:
            break;
        }
...
static ast::Expr *p_Additive() {
    ...
    while (next_token.type == TokenType::PLUS ||
           next_token.type == TokenType::MINUS) {
...
static ast::Expr *p_Multiplicative() {
    ...
    while (next_token.type == TokenType::MODULO ||
           next_token.type == TokenType::TIMES ||
           next_token.type == TokenType::DIVIDE) {
...
static ast::Expr *p_Unary() {
    /*      unary : Minus unary
                  ...
                  | Not unary
                  | primary           */
    if (isFirst[SymbolType::Primary]
               [next_token.type]) { // LPAREN,IDENTIFIER, integer constant
        /*
         isFirst[SvmbolType::Primary][next.token.type]用来判断下一个token 是否在
        FIRST(primary)中,以选择产生式unary:primary。
        在同学们需要填空的代码中，待选择的多个产生式右端通常都以终结符开始，因此只需通过判断
        ...
        */
        ...
        if (next_token.type == TokenType::MINUS) { // negative -
            ...
            return new ast::NegExpr(
                p_Unary(),
                minus.loc); // p_Unary: recursively parse a Unary symbol
    ...
    mind::err::issue(next_token.loc,
                     new mind::err::SyntaxError("expect unary expression get" +
                                                TokenName[next_token.type]));
    return NULL; // not to be execueted, only write this to eliminate compiler
                 // warnings.
}
...
static ast::Expr *p_Primary() {
    ...
    if (next_token.type == TokenType::ICONST) {
        ...
        Token name = lookahead(TokenType::IDENTIFIER);
        return new ast::LvalueExpr(new ast::VarRef(name.id, name.loc),
                                   name.loc);
    }
    mind::err::issue(next_token.loc, new mind::err::SyntaxError(
                                         "expect primary expression get" +
                                         TokenName[next_token.type]));
    return NULL;
}
